...
import xades4j.providers.TimeStampVerificationProvider.TimeStampValidationResult;
...
abstract class TimeStampVerifierBase<TData extends BaseXAdESTimeStampData>
	...
	public final QualifyingProperty verify(TData propData, QualifyingPropertyVerificationContext ctx)
		...
		try {
			...
			TimeStampProperty prop = addPropSpecificTimeStampInputAndCreateProperty(propData, digestInput, ctx);
			byte[] data = digestInput.getBytes();
			/**
			 * Verify the time-stamp tokens on a time-stamp property data
			 * object. All the tokens are verified, but the returned time-stamp
			 * is from the last token.
			 */
			List<byte[]> tokens = propData.getTimeStampTokens();
			TimeStampValidationResult ts = null;
			for (byte[] tkn : tokens) {
				ts = this.tsVerifier.verifyToken(tkn, data);
			}

			prop.setTime(ts.getDate());
			prop.setValidationData(ts.getValidationData());
			return prop;
	...
	protected abstract TimeStampProperty addPropSpecificTimeStampInputAndCreateProperty(TData propData,
